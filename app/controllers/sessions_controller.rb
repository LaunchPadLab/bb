class SessionsController < UIViewController
  ...
  def viewDidLoad
    ...
    # label.sizeToFit
    # label.center = [self.view.frame.size.width / 2, 200]
    # self.view.addSubview(label)
    
    logo_image = UIImage.imageNamed('logo.png')
    logo_view = UIImageView.alloc.initWithImage(logo_image)
    logo_view.center = [self.view.frame.size.width / 2, 170]
    ...
    label2.sizeToFit
    label2.center = [self.view.frame.size.width / 2, logo_view.frame.origin.y + logo_view.frame.size.height + 20]
    ...
    login_button.setTitle('Enter', forState:UIControlStateNormal)
    login_button.setTitle('Entering...', forState:UIControlStateDisabled)
    login_button.sizeToFit
    login_button.frame = [[60, 370], [200, 30]]
